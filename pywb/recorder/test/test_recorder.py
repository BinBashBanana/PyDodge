...
class TestRecorder(LiveServerTests, HttpBinLiveTests, FakeRedisTests,
                   TempDirTests, BaseTestClass):
    ...
    def _test_all_warcs(self, dirname, num):
        ...
        files = [
            x for x in os.listdir(coll_dir)
            if os.path.isfile(os.path.join(coll_dir, x))
        ]
    ...
    def _get_http_only_cookies(self, record):
        ...
        for header in record.http_headers.headers:
            name = header[0].lower()
            if name == 'set-cookie':
                if ExcludeHttpOnlyCookieHeaders.HTTPONLY_RX.search(
                        header[1].lower()):
                    http_only = header
                else:
        ...
        for filename in files:
            ...
            with open(filename, 'rb') as fh:
                for record in ArchiveIterator(fh, no_record_parse=True):
                    ...
                    assert int(record.rec_headers.get_header(
    ...
    def test_record_cookies_header(self):
        ...
        assert ('Set-Cookie',
                'name=value; Path=/') in stored_resp.http_headers.headers
        assert ('Set-Cookie',
                'foo=bar; Path=/') in stored_resp.http_headers.headers
    ...
    def test_record_skip_wrong_coll(self):
        recorder_app = RecorderApp(self.upstream_url,
                                   writer=PerRecordWARCWriter(
                                       to_path(self.root_dir + '/warcs/')),
                                   accept_colls='not-live')
    ...
    def test_record_param_user_coll_same_dir(self):
        ...
        recorder_app = RecorderApp(
            self.upstream_url,
            PerRecordWARCWriter(warc_path,
                                dedup_index=dedup_index,
                                key_template='{user}:{coll}'))

        resp = self._test_warc_write(
            recorder_app, 'httpbin.org', '/get?foo=bar',
            '&param.recorder.user=USER2&param.recorder.coll=COLL2')
        assert b'HTTP/1.1 200 OK' in resp.body
        assert b'"foo": "bar"' in resp.body

        resp = self._test_warc_write(
            recorder_app, 'httpbin.org', '/get?foo=bar',
            '&param.recorder.user=USER2&param.recorder.coll=COLL3')
        assert b'HTTP/1.1 200 OK' in resp.body
        assert b'"foo": "bar"' in resp.body
