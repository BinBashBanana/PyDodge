...
class TestIndexSources(FakeRedisTests, BaseTestClass):
    ...
    def setup_class(cls):
        ...
        cls.all_sources = {
            ...
            'remote_cdx':
            RemoteIndexSource(
                'https://webenact.rhizome.org/all/cdx?url={url}',
                'https://webenact.rhizome.org/all/{timestamp}id_/{url}'),
            'memento':
            MementoIndexSource(
                'https://webenact.rhizome.org/all/{url}',
    ...
    def test_local_cdxj_loader(self, local_source):
        ...
        res, errs = self.query_single_source(local_source,
                                             dict(url=url, limit=3))
...
org,iana)/_css/2013.1/fonts/inconsolata.otf 20140126200930 iana.warc.gz"""
    ...
    def test_local_closest_loader(self, local_source):
        url = 'http://www.iana.org/_css/2013.1/fonts/Inconsolata.otf'
        res, errs = self.query_single_source(
            local_source, dict(url=url, closest='20140126200930', limit=3))
...
org,iana)/_css/2013.1/fonts/inconsolata.otf 20140126200826 iana.warc.gz"""
    ...
    def test_file_prefix_loader(self, local_source):
        res, errs = self.query_single_source(
            local_source, dict(url='http://iana.org/domains/root/*'))
...
com,instagram)/amaliaulman 20141014171954 https://webenact.rhizome.org/all/20141014171954id_/http://instagram.com/amaliaulman"""
    ...
    def test_remote_loader_with_prefix(self):
        ...
        res, errs = self.query_single_source(
            remote_source,
            dict(url=url, closest='20141014162332', limit=1, allowFuzzy='0'))
...
com,instagram)/amaliaulman 20141014162333 https://webenact.rhizome.org/all/20141014162333id_/http://instagram.com/amaliaulman"""
    ...
    def test_remote_closest_loader(self, remote_source):
        url = 'http://instagram.com/amaliaulman'
        res, errs = self.query_single_source(
            remote_source, dict(url=url, closest='20141014162332', limit=1))
...
com,instagram)/amaliaulman 20141014162333 https://webenact.rhizome.org/all/20141014162333id_/http://instagram.com/amaliaulman"""
    ...
    def test_remote_closest_wb_memento_loader(self):
        ...
        url = 'http://instagram.com/amaliaulman'
        res, errs = self.query_single_source(
            source, dict(url=url, closest='20141014162332', limit=1))
...
com,instagram)/amaliaulman 20141014162333 https://webenact.rhizome.org/all/20141014162333id_/http://instagram.com/amaliaulman"""
    ...
    def test_live(self):
        ...
        expected = 'com,example)/ {0} http://example.com/'.format(
            timestamp_now())
    ...
    def test_all_not_found(self, all_source):
        ...
        res, errs = self.query_single_source(all_source, dict(url=url,
                                                              limit=3))
    ...
    def test_another_remote_not_found(self):
        source = MementoIndexSource.from_timegate_url(
            'https://webenact.rhizome.org/all/')
        url = 'http://x-not-found-x.notfound/'
        ...
        assert (
            errs['source'] ==
            "NotFoundException('https://webenact.rhizome.org/all/timemap/link/http://x-not-found-x.notfound/',)"
        )
    ...
    def test_file_not_found(self):
        ...
        expected = ''
        assert (key_ts_res(res) == expected)
        assert (errs['source'] == "NotFoundException('testdata/not-found-x',)"
                ), errs

    def test_ait_filters(self):
        ait_source = RemoteIndexSource(
            'http://wayback.archive-it.org/cdx/search/cdx?url={url}&filter=filename:ARCHIVEIT-({colls})-.*',
            'http://wayback.archive-it.org/all/{timestamp}id_/{url}')

        cdxlist, errs = self.query_single_source(
            ait_source, {
                'url': 'http://iana.org/',
                'param.source.colls': '5610|933'
            })
        ...
        cdxlist, errs = self.query_single_source(
            ait_source, {
                'url': 'http://iana.org/',
                'param.source.colls': '1883|366|905'
            })
        filenames = [cdx['filename'] for cdx in cdxlist]
