...
class TestRedisAgg(FakeRedisTests, BaseTestClass):
    ...
    def setup_class(cls):
        ...
        cls.add_cdx_to_redis(TEST_CDX_PATH + 'example2.cdxj',
                             'FOO:example:cdxj')
        cls.add_cdx_to_redis(TEST_CDX_PATH + 'dupes.cdxj', 'FOO:dupes:cdxj')

        # scan loader
        cls.scan_loader = RedisMultiKeyIndexSource(
            'redis://localhost/2/{user}:{coll}:cdxj')

        cls.redis.sadd('FOO:<all>:list', 'dupes')
        cls.redis.sadd('FOO:<all>:list', 'example')

        cls.member_list_loader = RedisMultiKeyIndexSource(
            'redis://localhost/2/{user}:{coll}:cdxj',
