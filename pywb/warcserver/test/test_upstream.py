...
class TestUpstream(LiveServerTests, HttpBinLiveTests, BaseTestClass):
    ...
    def test_upstream_paths(self):
        ...
        assert set(res.json.keys()) == {
            '/upstream/postreq', '/upstream', '/upstream_opt',
            '/upstream_opt/postreq'
        }
    ...
    def test_upstream_2(self):
        resp = self.testapp.get(
            '/upstream_opt/resource?url=http://httpbin.org/get')
        assert resp.headers[
            'Warcserver-Source-Coll'] == 'upstream_opt:live', resp.headers
