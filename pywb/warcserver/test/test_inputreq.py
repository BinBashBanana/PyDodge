...
class InputReqApp(object):
    ...
    def __init__(self):
        self.url_map = Map()
        self.url_map.add(
            Rule('/test/<path:url>', endpoint=self.direct_input_request))
...
class TestInputReq(object):
    ...
    def test_get_direct(self):
        res = self.testapp.get('/test/http://example.com/',
                               headers={'Foo': 'Bar'})
        assert res.text == '\
...
'
    ...
    def test_post_direct(self):
        res = self.testapp.post('/test/http://example.com/',
                                headers={'Foo': 'Bar'},
                                params='ABC')
...
'
        ...
        res = self.testapp.post('/test-postreq?url=http://example.com/',
                                params=postdata)
...
class TestPostQueryExtract(object):
    ...
    def test_post_extract_length_invalid_ignore(self):
        ...
        assert mq.append_query(
            'http://example.com/') == 'http://example.com/?__wb_method=POST'
    ...
    def test_amf_parse(self):
        mq = MethodQueryCanonicalizer('POST', 'application/x-amf', 0,
                                      BytesIO())
