...
class Amf:
    ...
    def get_representation(request_object, max_calls=500):
        ...
        if max_calls < 0:
            raise Exception(
                "Amf.get_representation maximum number of calls reached")
        ...
        if isinstance(request_object, Envelope):
            ...
            return "<Request target={target}>{body}</Request>".format(
                **locals())
        ...
        elif isinstance(request_object, object) and hasattr(
